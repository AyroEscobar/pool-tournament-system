...
$0.85$, punishing the underdog far more than the ratings justify.
...
The distance property is exact. If $\mathrm{pos}(s)$ is a seed's leaf index in
$S(k)$, then two seeds meet no earlier than the round given by the height of their
lowest common ancestor, which is one plus the index of the highest differing bit of
their leaf indices. The application computes this as \texttt{seedMeetRound}: seeds
$1$ and $2$ meet only in the final, seeds $1$ through $4$ no earlier than the
semifinals. A single elimination of $k$ players is exactly $k-1$ matches, since each
...
drops are crossed in reversed order so that a player never meets, in the round they
drop into, anyone from the winners match they just came through: there is no
immediate rematch. The whole bracket is built as a list of matches already
...
Figure~\ref{fig:app} shows the double elimination bracket the application produces
for a field of thirteen (top eight seeded): the winners bracket, the losers bracket
with its alternating rounds, and the grand final. Fields up to thirty players keep
every guarantee: at $n=30$ the group stage is $29$ rounds and $435$ matches, all
five checks pass, and Elo is conserved.

\begin{figure}[h]
\centering
\includegraphics[width=0.94\textwidth]{report_de_bracket.png}
\caption{The application's double elimination view: winners bracket (seeded tree),
losers bracket (six matches over four rounds for $k=8$), and grand final.}
\label{fig:app}
\end{figure}
