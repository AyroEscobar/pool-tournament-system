...
5. Press Play on the K_n diagram. Each step slides every chord one ring seat (the rotation itself, animated), the dots pulse because every vertex is covered exactly once (the definition of a perfect matching), played edges stay as chalk residue, and the counter runs to "all 28 edges drawn exactly once". That is the one-factorization proving itself in front of you.
6. Switch Knockout format to Double elim, click the 13 example (it regenerates), and Start on Fast: the top 8 fill a winners bracket, first losses drop into the losers bracket, and the grand final (with a reset game if the losers champion takes it) crowns the champion. Everyone gets at least two games.
7. Click the example fields (8, 13, 30). Thirteen shows the rotating bye and n rounds for odd n; thirty shows the same guarantees at 29 rounds and 435 matches, verified live.
...
- E. Lucas, Recreations Mathematiques, volume 2, Gauthier-Villars, Paris, 1883.
